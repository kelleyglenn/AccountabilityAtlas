...
# Flatten JWT PEM to single line (\n literals) so it fits in a .env file.
# Docker Compose interprets \n inside double quotes as real newlines,
# and the Java PEM parser strips whitespace, so this round-trips correctly.
JWT_FLAT=$(printf '%s' "$JWT_PRIVATE_KEY" | awk '{printf "%s\\n", $0}' | sed 's/\\n$//')
...
# Convert to Unix line endings (Git Bash on Windows produces CRLF)
sed -i 's/\r$//' "$GENERATED_ENV"
